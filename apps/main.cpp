...
int main() {
    ...
    if (err != cudaSuccess) {
        ...
        std::cerr << "CUDA Runtime Error details: " << err << std::endl;
        std::cout << "CUDA devices found: 0 (Error occurred)" << std::endl;
    } else {
        std::cout << "CUDA devices found: " << deviceCount << std::endl;
    }

    // Test CUDA kernel (only if devices available)
    if (deviceCount > 0) {
        const int n = 1000;
        ...
        std::cout << "Note: No CUDA devices found. Running CPU-only version." << std::endl;
    }
